				...
				<template v-else>
					<div class="pool__wrapper__container">
						<div class="sp-component-title">
							<h3 class="truncate" :style="{ maxWidth: '50rem' }">
								Pair {{ pool.meta.name }}
							</h3>
						</div>
						...
						<dl class="pool__stats sp-box sp-shadow">
							<div
								class="pool__stats__item pool__stats__wallet"
								v-if="isLoggedIn"
							>
							...
							<div class="pool__stats__item" v-if="reserveBalances[0]">
								<dd class="pool__stats__reserve">
									<div class="pool__stats__reserve__denom truncate">
										{{ reserveBalances[0].amount
										}}{{ reserveBalances[0].denom }}
									</div>
									<div class="pool__stats__reserve__denom truncate">
										{{ reserveBalances[1].amount
										}}{{ reserveBalances[1].denom }}
									</div>
								</dd>
...
export default defineComponent({
	...
	setup() {
		...
		const shares = computed(() => {
			...
			// @ts-ignore
			const valueString = calculateShares({ amount: walletPoolBalance.value })
				?.percentage
			return valueString
				? new BigNumber(valueString)
						.multipliedBy(100)
						.decimalPlaces(4)
						.toString()
				: '0'
		...
		return {
			...
			poolSupply,
			shares,
			reserveBalances: computed(() =>
				Object.values(reserveBalances.value || {})
			)
		}
	}
})
